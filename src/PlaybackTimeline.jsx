...
export function PlaybackTimeline({ assignment, playback, onRun, onRestart, onScrub, duration, getTime, speed, onSpeed }) {
    ...
    <footer className="timeline">
      ...
      <div className="timeline-track" aria-label="Play timing">
        <div className="phase-labels">
          <span>Pre-snap</span>
          <strong>Snap</strong>
          <span>Assignments{selectedStart === null ? "" : ` · selected ${selectedStart.toFixed(2)}s`}<small ref={readoutRef} className="time-readout" /></span>
        </div>
        <div
          ref={trackRef}
          ...
          onPointerDown={(event) => { event.currentTarget.setPointerCapture?.(event.pointerId); scrubToPointer(event); }}
          onPointerMove={(event) => { if (event.buttons) scrubToPointer(event); }}
