...
test('phone PNG keeps readable legend and keyed owners without saving export or dimming state',async()=>{
 ...
 await page.evaluate(()=>{new MutationObserver(()=>{const svg=document.querySelector('.lesson-export svg[data-ready="true"]');if(!svg)return;const texts=[...svg.querySelectorAll('.responsibility-legend text')];window.__phoneExport={fieldSide:svg.querySelector('.field-side-indicator')?.textContent,width:svg.getBoundingClientRect().width,legend: texts.map(e=>({text:e.textContent,x:e.getAttribute('x'),font:parseFloat(getComputedStyle(e).fontSize)*svg.getBoundingClientRect().width/svg.viewBox.baseVal.width})),offense:getComputedStyle(svg.querySelector('g.player')).opacity,defense:getComputedStyle(svg.querySelector('g.defender[data-player="colors-cl"] circle')).fill,controls:svg.querySelectorAll('animateMotion,[data-region-handle],.route-arrow-active').length};}).observe(document.body,{childList:true,subtree:true,attributes:true});});
 const download=page.waitForEvent('download');await page.getByRole('button',{name:/Export phone PNG/}).click();const dl=await download;await mkdir(out,{recursive:true});await dl.saveAs(join(out,'phone-visuals.png'));
 const capture=await page.evaluate(()=>window.__phoneExport);assert.equal(capture.width,390);assert.equal(capture.fieldSide,'FIELD →');assert.equal(capture.legend.length,7);assert.equal(new Set(capture.legend.map(t=>t.x)).size,4);assert.ok(capture.legend.every(t=>t.font>=11.99));assert.ok(capture.legend.some(t=>t.text==='B — Left flat'));assert.ok(capture.legend.some(t=>t.text==='A — Right flat'));assert.equal(capture.offense,'0.3');assert.equal(capture.defense,'rgb(124, 183, 255)');assert.equal(capture.controls,0);
 const bytes=await readFile(join(out,'phone-visuals.png'));assert.equal(bytes.readUInt32BE(16),780);assert.ok(bytes.readUInt32BE(20)>bytes.readUInt32BE(16));
