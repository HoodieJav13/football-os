...
test('phone PNG keeps readable legend and keyed owners without saving export or dimming state',async()=>{
 ...
 const capture=await page.evaluate(()=>window.__phoneExport);assert.equal(capture.width,390);assert.equal(capture.fieldSide,'FIELD →');assert.equal(capture.legend.length,7);assert.equal(new Set(capture.legend.map(t=>t.x)).size,6);assert.ok(capture.legend.every(t=>t.font>=13.9));assert.ok(capture.legend.some(t=>t.text==='B — Left flat'));assert.ok(capture.legend.some(t=>t.text==='A — Right flat'));assert.equal(capture.offense,'0.3');assert.equal(capture.defense,'rgb(124, 183, 255)');assert.equal(capture.controls,0);
