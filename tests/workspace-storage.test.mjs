...
import { loadWorkspaceState, loadGameDayState, restoreWorkspace, recoverGameDay } from '../src/workspaceStorage.js';
...
test('game-day recovery preserves original bytes and resets only after successful writes',()=>{
 for(const key of ['football-os.game-day.v6','football-os.game-day.v7']){
  const s=storage({[key]:'{corrupt adjustment'}),state=loadGameDayState(s);
  recoverGameDay(s,state);
  assert.equal(JSON.parse(s.getItem('football-os.game-day-recovery.v1')).raw,'{corrupt adjustment');
  assert.equal(loadGameDayState(s).writable,true);assert.equal(loadGameDayState(s).gameDay,null);
  const failed=storage({[key]:'{corrupt adjustment'});failed.setItem=()=>{throw new Error('quota');};
  assert.throws(()=>recoverGameDay(failed,loadGameDayState(failed)),/quota/);
  assert.equal(failed.getItem(key),'{corrupt adjustment');assert.equal(loadGameDayState(failed).writable,false);
 }
});
test('a migrated custom-book collision keeps its game-day adjustment attached',()=>{
 const p=createDefaultWorkspace().playbooks[0].plays[0];
 const workspace=createDefaultWorkspace();
 const custom={...structuredClone(workspace.playbooks[0]),id:'air-raid-reference',name:'Custom',source:'personal',readOnly:false};
 workspace.playbooks=workspace.playbooks.filter(b=>b.id!=='air-raid-reference');workspace.playbooks.push(custom);
 const s=storage({[WORKSPACE_KEY]:JSON.stringify(workspace),'football-os.game-day.v6':JSON.stringify({playbookId:custom.id,playId:p.id,snapshot:p})});
 const loaded=loadWorkspaceState(s),game=loadGameDayState(s,loaded.workspace);
 assert.equal(game.gameDay.playbookId,loaded.workspace.playbooks.find(b=>b.name==='Custom').id);
 assert.deepEqual(game.gameDay.snapshot,p);
});
test('duplicate saved book IDs trigger recovery instead of cross-book edits',()=>{
 const w=createDefaultWorkspace();w.playbooks.push(structuredClone(w.playbooks[0]));
 assert.equal(loadWorkspaceState(storage({[WORKSPACE_KEY]:JSON.stringify(w)})).writable,false);
});
